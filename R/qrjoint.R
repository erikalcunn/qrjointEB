#library(qrjoint)    # Tokdar package    # must load and compile the C code, including "INIT" upon startup
#library(quantreg)   # Kroenker package
#library(splines)
#library(coda)
# Delete these libraries eventually

## version 2. Playing with the default for kappa

# note 5/19/16 documentation notes
# 1) qrjoint "hyper" could use some work... maybe just split these out into not a list
# 2) object, nadd, and append show up in documentation, but not in code???
# 3) Examples the "don't run" sections are weird

# note 5/20/16 code notes
#1) plot for coefficients should return user to pre settings settings
      # Changed 10/29/16
#2) coefficients should parse out beta.samp into it's separated p + 1 parameter posterior
#   sample blocks
      # Changed 10/19/16


# note 5/23/16 code note
# check that the uppermost quantile is doing the right thing.
# I have a crossing of lines (maybe that is just a result of doing the mean on too few samples?)


# note 5/24/16 code note
# getting this error "Error in names(beta.hat) <- plot.titles : 
# 'names' attribute [14] must be the same length as the vector [2]"
# when I run this on a simple linear regression case (Simulated1.R)
# it is not correctly converting the vector to a matrix and then it is 
# not calculating p correctly.  Also strange that the function is able to
# access variables outside of the function environment.  Do we need more
# protections on this???
      # this problem should go away with 10/19/16 changes
...
# Note 7/25/16 documentation note
# In manual documentation, "coef" should link to coef.qrjoint instead of the generic coef
# UPDATED NEED TO TEST

# Note 7/25/16 -- see the qrjoint_EB_20160316.R code
# Modified so that the beta.samp object that gets returned is a three dimensional array.
# Manual needs changing to reflect.

# Note 7/27/16
# coef needs catch handling to continue working even when the plot region is
# not large enough to accommodate all of the plots

# Note 8/10/16
# Confusing to have this list of the kap object (which becomes a.kap)
# And documentation does not make it any clearer nkap.
# a.kap has two sets of defaults.  Need to make consistent

# NOTES 10/19/16  Built a predict function
# QRJoint needs to have Warning handling for the RQ functions used to get starting values
# Need coherent way for getting density functions from these Quantile estimates...
# use medians as estimates???
# Need documentation for predict function

# NOTE 3/15/17
# Fixed naming of the beta.hat/beta.est object in the coef.qrjoint function
# so that it does not sometimes return a list.
...
qrjoint <- function(formula, data, nsamp = 1e3, thin = 10, cens = NULL, 
                    ...
                    blocks.mu, blocks.S, fix.nu = FALSE, fbase = c("t", "unif")){
    ...
    stop("Model formula must contain an intercept")
    
    if(sum(attr(mt,"order"))==0)
    stop("Model formula must contain at least one non-intercept term")
    ...
    if(is.null(cens)) cens <- rep(0,length(y)) else{
      cens <- as.vector(cens)
        if(!is.numeric(cens))
          stop("'cens' must be a numeric vector")
      
        if (length(cens) != NROW(y)) 
          stop(gettextf("number of cens is %d, should equal %d (number of observations)", 
                      length(cens), NROW(y)), domain = NA)
    }
    
    if(is.null(wt)) wt <- rep(1,length(y)) else{
      wt <- as.vector(wt)
      if(!is.numeric(wt))
...
# Things that I don't like about this code...

# Not automatically transforming to beta coefficients in the qrjoint function....
# but I can be okay with that, especially since it keeps everything closer to the 
# form (w's etc) that can be updated (via the update function)

# Same goes for not returning the X matrix put back on its original scale -
# again likely left as is in the qrjoint function for easier qrjoint.update
# However, I do think that this could be returned from qrjoint.coef -- or just go straight to predict

# I think that I would like to separate out the plotting from the 
# coef.qrjoint function.... biggest problem is that plotting is done via "getbands" function
# and so therefore not easier to sort everything out.

# coef.qrjoint returns one as a 3-D array and one as a list, should I make this more consistent?

# I think that a 3-dimensional array for returns seems the logcial thing to do
# because everything is arranged by coefficient, taugrid, and iteration
# then can slice to get get one iteration (all coefficient, taugrids, a posterior predictive draw)
# or by taugrid
# or covariate (to summarize across iterations and get plots)

# Could I have a separate plot.coef function that refers to the get.bands function?
...
coef.qrjoint <- function(object, burn.perc = 0.5, nmc = 200, plot = FALSE, show.intercept = TRUE, reduce = TRUE, ...){
  ...
  if(plot) suppressWarnings(par(cur.par))  # return R parameters to pre-plot settings
...
# summarize:  Logical - return medians & 95% summarized MC posterior quantiles
